{{-- <div class="min-h-screen bg-gray-50 dark:bg-neutral-900">
    <!-- Header -->
    <div class="bg-gradient-to-r from-green-600 to-indigo-800 shadow-lg">
...
<x-livewire.layout>
    ...
    <x-slot name="header">
        <x-livewire.components.header name="Budgets" description="Plan and track your spending limits" :withCalender="true"
            from_to_color="from-green-600 to-indigo-800" selectedMonth="{{ $selectedMonth }}"
            previous_month_function_name="previousMonth" selectedYear="{{ $selectedYear }}" />
    </x-slot>
    <div class="max-w-7xl mx-auto py-8">
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-purple-500">
                ...
                @if ($monthlyBudget > 0)
                    <div class="mt-2 text-sm {{ $totalSpent > $monthlyBudget ? 'text-red-600' : 'text-green-600' }}">
                        {{ $totalSpent > $monthlyBudget ? 'Over' : 'Under' }} budget by
                        ${{ number_format(abs($monthlyBudget - $totalSpent), 2) }}
                    </div>
                @endif
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <canvas id="monthlyTrendChart" data-months='@json($monthlyComparison->pluck('month'))'
                    data-totals='@json($monthlyComparison->pluck('total'))'></canvas>
            </div>
            <div class="bg-white rounded-xl shadow-md p-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Spending by Category</h3>
                <canvas id="categoryChart" data-labels='@json($expenseByCategory->pluck('name'))'
                    data-values='@json($expenseByCategory->pluck('total'))' data-colors='@json($expenseByCategory->pluck('color'))'></canvas>
            </div>
        </div>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="/expenses/create"
                class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl p-6 hover:shadow-lg transition transform hover:-translate-y-1">
                <div class="flex items-center gap-4">
            ...
            <a href="/recurring-expenses"
                class="bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl p-6 hover:shadow-lg transition transform hover:-translate-y-1">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-white/20 rounded-lg">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="/categories"
                class="bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl p-6 hover:shadow-lg transition transform hover:-translate-y-1">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-white/20 rounded-lg">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            const initialTrendCanvas = document.getElementById('monthlyTrendChart');
            const initialCategoryCanvas = document.getElementById('categoryChart');
            chartPayload = {
                months: JSON.parse(initialTrendCanvas?.dataset.months || '[]'),
                totals: JSON.parse(initialTrendCanvas?.dataset.totals || '[]'),
                labels: JSON.parse(initialCategoryCanvas?.dataset.labels || '[]'),
                values: JSON.parse(initialCategoryCanvas?.dataset.values || '[]'),
                colors: JSON.parse(initialCategoryCanvas?.dataset.colors || '[]'),
            };
