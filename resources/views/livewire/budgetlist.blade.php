<x-livewire.layout>
    ...
    <x-slot name="header">
        <x-livewire.components.header name="Budgets" description="Plan and track your spending limits" :withCalender="true"
            from_to_color="from-indigo-600 to-purple-600" selectedMonth="{{ $selectedMonth }}"
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-md p-6 mb-8 ">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div>
                <p class="text-sm text-gray-600 dark:text-gray-200 mb-1">Remaining</p>
                <p class="text-2xl font-bold {{ $totalRemainingAmount < 0 ? 'text-red-600' : 'text-green-600' }}">
                    ${{ number_format(abs($totalRemainingAmount), 2) }}
            ...
            <div>
                <p class="text-sm text-gray-600 dark:text-gray-200 mb-1">Usage</p>
                <p
                    class="text-2xl font-bold {{ $overallUsingPercentage > 100 ? 'text-red-600' : ($overallUsingPercentage > 80 ? 'text-yellow-600' : 'text-gray-900 dark:text-gray-200') }}">
                    {{ number_format($overallUsingPercentage, 1) }}%
                </p>
            </div>
        </div>
        <!-- Overall Progress Bar -->
        <div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4">
            <div class="h-4 rounded-full transition-all duration-300 {{ $overallUsingPercentage > 100 ? 'bg-red-500' : ($overallUsingPercentage > 90 ? 'bg-orange-500' : ($overallUsingPercentage > 80 ? 'bg-yellow-500' : 'bg-green-500')) }}"
                style="width: {{ min($overallUsingPercentage, 100) }}%"></div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a wire:navigate href="{{ route('budgets.create') }}"
                ...
                <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-md overflow-hidden hover:shadow-xl transition"
                    ...
                    <div class="p-6 {{ $budget->category ? '' : 'bg-gradient-to-r from-gray-500 to-gray-600' }}"
                        ...
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                <a wire:navigate href="{{ route('budgets.edit', $budget->id) }}"
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="text-2xl font-bold text-gray-900 dark:text-gray-200">${{ number_format($budget->amount, 2) }}</span>
                        </div>
                        ...
                        <div class="pt-2 border-t border-gray-200 space-y-2">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span
                                    class="font-semibold text-gray-900 dark:text-gray-200">${{ number_format($budget->spent, 2) }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                        ...
                        <div class="pt-2">
                            @if ($budget->is_over)
                                <div
                                    class="flex items-center gap-2 px-3 py-2 bg-red-100 border border-red-200 rounded-lg">
                            ...
                            @else
                                <div
                                    class="flex items-center gap-2 px-3 py-2 bg-green-100 border border-green-200 rounded-lg">
